<script lang="ts">
  ...
  let { tables, policies, currentPolicyId, selectedTable, currentSchema, availableSchemas, schemaLoading, connected, currentDatabase, testsByTable = new Map(), onSelectTable, onSelectPolicy, onDeletePolicy, onChangeSchema, onOpenConnectionDialog, onSelectTests, onSelectData }: Props = $props();

  // Track expanded tables
  let expandedTables = $state<Set<string>>(new Set());
  // Track expanded sub-branches (policies, tests) per table
          ...
          {#if expandedTables.has(fullName)}
            <div class="pl-5 border-l border-border ml-4">
              ...
              <div class="tree-branch">
                <div
                  class="flex items-center gap-1.5 ml-6 px-1 py-1 hover:bg-muted/50 transition-colors cursor-pointer"
                  role="button"
                  tabindex="0"
                  onclick={() => handleSelectData(fullName)}
                  onkeydown={(e) => e.key === 'Enter' && handleSelectData(fullName)}
                >
                  <TableIcon class="h-3 w-3 text-amber-500 shrink-0" />
                  <span class="text-xs text-foreground">Data</span>
                </div>
              </div>
